...
extern const unsigned char uni2cubechars[878] =
{
...
size_t decodeutf8(unsigned char *dstbuf, size_t dstlen, const unsigned char *srcbuf, size_t srclen, size_t *carry)
{
    unsigned char *dst = dstbuf, *dstend = &dstbuf[dstlen];
    const unsigned char *src = srcbuf, *srcend = &srcbuf[srclen];
    if(dstbuf == srcbuf)
    {
        int len = std::min(dstlen, srclen);
        for(const unsigned char *end4 = &srcbuf[len&~3]; src < end4; src += 4) if(*(const int *)src & 0x80808080) goto decode;
        for(const unsigned char *end = &srcbuf[len]; src < end; src++) if(*src & 0x80) goto decode;
        if(carry) *carry += len;
        return len;
    }
...
size_t encodeutf8(unsigned char *dstbuf, size_t dstlen, const unsigned char *srcbuf, size_t srclen, size_t *carry)
{
    unsigned char *dst = dstbuf, *dstend = &dstbuf[dstlen];
    const unsigned char *src = srcbuf, *srcend = &srcbuf[srclen];
    if(src < srcend && dst < dstend) do
    {
        ...
        if(uni <= 0x7F)
        {
            ...
            const unsigned char *end = std::min(srcend, &src[dstend-dst]);
            do
            {
                if(uni == '\f')
                {
